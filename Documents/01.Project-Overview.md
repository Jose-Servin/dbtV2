...
We were also able to see this compiled dbt_expectations sql in the terminal by running `dbt --debug test --select dim_listings_w_hosts`.
...
Note: `start_date` and `end_date` are datetimes so we must pass `
